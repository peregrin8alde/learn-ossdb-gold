...
classDiagram
  ...
  SQL の DDL で定義される各オブジェクト <|-- ロール _ ROLE _ GROUP
  ...
  SQL の DDL で定義される各オブジェクト <|-- テーブル
  SQL の DDL で定義される各オブジェクト <|-- TABLESPACE
  SQL の DDL で定義される各オブジェクト <|-- TEXT SEARCH 系 _ CONFIGURATION _ DICTIONARY _ PARSER _ TEMPLATE
